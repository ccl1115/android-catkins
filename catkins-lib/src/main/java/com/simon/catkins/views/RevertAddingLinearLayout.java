...
public class RevertAddingLinearLayout extends LinearLayout {
    ...
    private class DefaultTransitionAnimation implements ViewGroupInjector {
        ...
        public DefaultTransitionAnimation() {
            ...
            mProportionVelocity = densitiy * ViewConfig.PROPORTION_VELOCITY_SMALL;
        }
        ...
        private void computeAdding() {
            final long now = SystemClock.uptimeMillis();
            final float t = (now - mLastAnimationTime) / ViewConfig.ONE_SECOND_FLOAT;
            ...
            if (mAnimatingProportion >= 1f) {
                ...
                mInterpolatedPropotion = ViewConfig.sInterpolator.getInterpolation(mAnimatingProportion);
                mLastAnimationTime = now;
                mCurrentAnimatingTime = now + ViewConfig.ANIMATION_FRAME_DURATION;
                mHandler.sendEmptyMessageAtTime(MSG_ADD_VIEW, mCurrentAnimatingTime);
            }
        ...
        private void computeRemoving() {
            ...
            if (mAnimatingProportion <= 0f) {
                ...
                mInterpolatedPropotion = ViewConfig.sReverseInterpolator.getInterpolation(mAnimatingProportion);
                mLastAnimationTime = now;
                mCurrentAnimatingTime += ViewConfig.ANIMATION_FRAME_DURATION;
                mHandler.sendEmptyMessageAtTime(MSG_REMOVE_VIEW, mCurrentAnimatingTime);
            }
            invalidate();
        }
